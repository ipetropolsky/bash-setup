...
1) Добавляем в `~/.bashrc` запуск `setup.sh` из этого репозитория, например так:
```bash
# Запускаем в папке репозитория
printf '%s\n' \
    '' \
...
2) Чтобы изменения применились, открываем новый терминал ЛИБО запускаем в текущем:
```bash
source ~/.bashrc
```
